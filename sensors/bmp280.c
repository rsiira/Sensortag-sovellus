...
void bmp280_setup(I2C_Handle *i2c) {
	...
	/*Tekee konversion kolmesta 8-bittisesta rekisterinarvosta msb,lsb 
	ja xlsb ilmanpainearvoon muodostaen 20-bittisen binaariluvun:
	press_msb[7:0],press_lsb[7:0],press_xlsb[7:4].*/

	uint32_t bitti_20 = 0;
	
	//xlsb[7:4] bittien irroitus maskilla.
	uint8_t xlsb_maski = 0b11110000;
	uint8_t xlsb_valitulos = xlsb & xlsb_maski;
	uint8_t xlsb_7_4 = xlsb_valitulos >> 4;
	
	//20-bittisen binaariluvun kokoaminen. 
	uint32_t valitulos_1 = msb << 8; //press_msb[7:0], 00000000
	uint32_t valitulos_2 = valitulos_1 | lsb; //press_msb[7:0],press_lsb[7:0]
	uint32_t valitulos_3 = valitulos_2 << 4; // press_msb[7:0],press_lsb[7:0], 0000
	bitti_20 = valitulos_3 | xlsb_7_4; //press_msb[7:0],press_lsb[7:0],press_xlsb[7:4]
...
void bmp280_get_data(I2C_Handle *i2c, double *pres, double *temp) {
	...
	char str_lampotila[24];
	char str_ilmanpaine[24];

	// JTKJ: Uncomment and fill in the data structure below with correct values..
	
    uint8_t txBuffer[ 1 ];
    uint8_t rxBuffer[ 6 ];
   txBuffer[0] = BMP280_REG_PRES; // Laiterekisteri, mihin operaatio kohdistuu
   i2cTransaction.slaveAddress = Board_BMP280_ADDR; // No nyt laitteen osoite 
   ...
   i2cTransaction.readCount = 6; // Vastaanotetaan kaksi tavua
    ...
    uint32_t ilmanpaine_20_bitti = bitti_20(rxBuffer[ 2 ], rxBuffer[ 1 ], rxBuffer[ 0 ]);
    uint32_t lampotila_20_bitti = bitti_20(rxBuffer[ 5 ], rxBuffer[ 4 ], rxBuffer[ 3 ]);
	...
	*temp = (bmp280_temp_compensation(lampotila_20_bitti) - 32.0) / 1.8;
	
	*pres = bmp280_convert_pres(ilmanpaine_20_bitti) / 100;
	
	/*sprintf(str_lampotila,"Lampotila on %.2f C.\n", *temp); 
	sprintf(str_ilmanpaine,"Ilmanpaine on %.2f hPa.\n", *pres);
    System_printf(str_lampotila);
    System_printf(str_ilmanpaine);
    System_flush();*/
